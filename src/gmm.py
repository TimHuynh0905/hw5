...
class GMM():
    ...
    def _e_step(self, features):
        ...
        """
        assignments = np.zeros((features.shape[0], self.n_clusters))
        
        for k_idx in range(self.n_clusters):
            assignments[:, k_idx] = self._posterior(features, k_idx)

        # return assignments
        return assignments / assignments.sum(axis = 1, keepdims = True)
    ...
    def _m_step(self, features, assignments):
        ...
        """
        # Update mixing_weights
        self.mixing_weights = np.mean(assignments, axis = 0)
        # self.mixing_weights = np.sum(assignments, axis = 0) / features.shape[0]
        
        # Update means
        self.means = np.dot(assignments.T, features) / np.sum(assignments, axis = 0).reshape(-1, 1)

        # Update covariances
        for j in range(self.n_clusters):
            self.covariances = np.zeros(self.covariances.shape)
            for i in range(features.shape[0]):
                print(assignments[i, j] * np.square(features[i] - self.means[j]) / np.sum(assignments, axis = 0)[j])
                self.covariances[j] += assignments[i, j] * np.square(features[i] - self.means[j]) / np.sum(assignments, axis = 0)[j]
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        """
        gmm_prob = np.log(self.mixing_weights[k_idx]) + multivariate_normal.logpdf(features, self.means[k_idx], self.covariances[k_idx])
        return gmm_prob
